...
import netty.smpp.api.event.*;
import netty.smpp.api.message.*;
import netty.smpp.api.message.tlv.Tag;
import netty.smpp.api.net.SmscLink;
import netty.smpp.api.net.TcpLink;
import netty.smpp.api.util.*;
import netty.smpp.api.version.SMPPVersion;
import netty.smpp.api.version.VersionException;
...
public class Connection implements Runnable {
     ...
     * constructor that will create a new {@link netty.smpp.api.net.TcpLink}object
     ...
     * communication desired. See the {@link netty.smpp.api.Connection}class description for
     ...
     *
     * @param alphabet the alphabet to use as the default for this connection (may be
     ...
     *
     * @see netty.smpp.api.Connection#getState
     */
     ...
     * @throws netty.smpp.api.version.VersionException if an attempt is made to set the version of the connection
     *                                                 after binding to the SMSC.
     * @see netty.smpp.api.version.SMPPVersion
     */
     ...
     *
     * @throws java.io.IOException if an i/o error occurs while opening the connection.
     */
     ...
     *
     * @throws netty.smpp.api.IllegalStateException if an attempt is made to close the connection while bound to
     *                                              the SMSC.
     * @throws java.io.IOException                  if an I/O exception occurs while trying to close the link.
     * @see netty.smpp.api.net.SmscLink#close
     */
     ...
     *
     * @see #setInterfaceVersion(netty.smpp.api.version.SMPPVersion)
     */
     ...
     * @throws java.io.IOException If an I/O error occurs writing the response packet to the
     *                             network connection.
     */
     ...
     * default, the {@link netty.smpp.api.util.DefaultSequenceScheme}class is used
     ...
     * @throws java.net.SocketTimeoutException              If a socket timeout occurs while waiting for a response
     *                                                      packet. (Only in synchronized mode).
     * @throws java.io.IOException                          If an I/O error occurs while writing the request packet to
     *                                                      the network connection.
     * @throws netty.smpp.api.UnsupportedOperationException If this connection type does not support operation
     *                                                      <code>r</code>. For example, a receiver link does not
     *                                                      support the submit_sm operation.
     * @throws netty.smpp.api.AlreadyBoundException         If the request type is a bind packet and this connection is
     *                                                      already bound.
     * @throws netty.smpp.api.message.SMPPProtocolException If synchronous communications is in use and the incoming
    ...
    public SMPPResponse sendRequest(SMPPRequest request)
            ...
            netty.smpp.api.AlreadyBoundException, VersionException, SMPPProtocolException,
            netty.smpp.api.UnsupportedOperationException {
     ...
     *
     * @throws netty.smpp.api.version.VersionException if the version in use does not support the request being
     *                                                 sent.
     */
     ...
     * <code>req</code>.
     * @throws java.net.SocketTimeoutException if the read on the socket times out.
     * @see #readNextPacket
     ...
     * @throws java.io.IOException If an I/O error occurs while writing the response packet to
     *                             the output stream.
     */
    ...
    public BindResp bind(int type,
            ...
            IllegalArgumentException, netty.smpp.api.AlreadyBoundException, VersionException,
            SMPPProtocolException {
     ...
     * @throws IllegalArgumentException                      if a bad <code>type</code> value is supplied.
     * @throws netty.smpp.api.VersionException               if an attempt is made to bind as transceiver while using SMPP
     *                                                       version 3.3.
     * @throws netty.smpp.api.InvalidParameterValueException If any of systemID, password, system type or address range
     *                                                       are outside allowed bounds or the TON or NPI is invalid.
     * @throws java.io.IOException                           If an I/O error occurs while writing the bind packet to the
     *                                                       output stream.
     * @throws netty.smpp.api.AlreadyBoundException          If the Connection is already bound.
     * @throws netty.smpp.api.SMPPProtocolExcpetion          if synchronous communication is in use and the incoming
    ...
    public BindResp bind(int type,
        ...
        } catch (netty.smpp.api.BadCommandIDException x) {
            LOGGER.error("Internal error in the smppapi. Please inform the maintainer.", x);
        }
     ...
     * is being used.
     * @throws netty.smpp.api.NotBoundException             if the connection is not yet bound.
     * @throws netty.smpp.api.message.SMPPProtocolException If synchronous comms is in use and the incoming unbind_resp
     *                                                      packet violates the SMPP specification, this exception is
     *                                                      thrown.
     * @throws java.io.IOException                          If an I/O error occurs while writing the unbind request or
     *                                                      reading the unbind response.
     */
    public UnbindResp unbind() throws IOException, NotBoundException,
            SMPPProtocolException {
     ...
     *
     * @throws netty.smpp.api.NotBoundException     if the connection is not currently bound.
     * @throws netty.smpp.api.AlreadyBoundException if no unbind request has been received from the SMSC.
     * @throws java.io.IOException                  If an I/O error occurs while writing the response packet to
     *                                              the network connection.
     */
    public void unbind(UnbindResp ubr) throws IOException,
            SMPPException {
        if (state != UNBINDING) {
        ...
        if (!(link.isConnected())) {
            throw new netty.smpp.api.AlreadyBoundException("No unbind request received.");
        }
     ...
     *
     * @throws java.io.IOException If an I/O error occurs while writing to the network
     *                             connection.
     */
     ...
     */
    public EnquireLinkResp enquireLink() throws IOException,
            SMPPProtocolException {
        try {
     ...
     *
     * @throws netty.smpp.api.AlreadyBoundException if the connection is currently bound to the SMSC.
     */
    public void reset() throws netty.smpp.api.AlreadyBoundException {
        if (state == BOUND) {
            LOGGER
     ...
     * have. By default, {@link netty.smpp.api.util.DefaultSequenceScheme}is used,
     ...
     *
     * @see netty.smpp.api.util.SequenceNumberScheme
     * @see netty.smpp.api.message.SMPPPacket#setSequenceNum
     */
     ...
     * @throws java.io.IOException                          If an I/O error occurs while reading from the network
     *                                                      connection.
     * @throws netty.smpp.api.InvalidOperationException     If asynchronous comms is in use.
     * @throws netty.smpp.api.message.SMPPProtocolException if the incoming data violates the SMPP protocol
     *                                                      specifications.
     */
    public SMPPPacket readNextPacket() throws IOException,
    ...
    private SMPPPacket readNextPacketInternal()
            throws IOException, SMPPProtocolException {
        try {
            SMPPPacket pak = null;
            int id = -1;
     ...
     * event dispatcher, this method will call {@link netty.smpp.api.event.EventDispatcher#init}to
     ...
     * new event dispatcher is in place, the {@link netty.smpp.api.event.EventDispatcher#destroy}
     ...
     * @return a subclass of {@link netty.smpp.api.message.SMPPPacket}
     * corresponding to SMPP command <code>commandId</code>.
     * @throws netty.smpp.api.BadCommandIDException if the command ID is not recognised.
     * @throws netty.smpp.api.NotSupportedException if the Connection is currently using an SMPP version which
