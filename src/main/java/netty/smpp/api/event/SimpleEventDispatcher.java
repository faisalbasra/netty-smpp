 ...
 * @see netty.smpp.api.event.EventDispatcher
 */
public class SimpleEventDispatcher implements netty.smpp.api.event.EventDispatcher {
     ...
     */
    private netty.smpp.api.event.ConnectionObserver[] observers;

    /**
     * Create a new SimpleEventDispatcher.
     */
    public SimpleEventDispatcher() {
        observers = new netty.smpp.api.event.ConnectionObserver[INCREMENT];
    }

    /**
     * Create a new SimpleEventDispatcher and register one observer on it.
     */
    public SimpleEventDispatcher(netty.smpp.api.event.ConnectionObserver ob) {
        observers = new netty.smpp.api.event.ConnectionObserver[1];
        addObserver(ob);
    }
     ...
     *
     * @param ob the ConnectionObserver object to add.
     */
    public void addObserver(netty.smpp.api.event.ConnectionObserver ob) {
     ...
     *
     * @param ob The ConnectionObserver object to remove.
     */
    public void removeObserver(netty.smpp.api.event.ConnectionObserver ob) {
        synchronized (observers) {
     ...
     * events.
     *
     * @param ob The ConnectionObserver to check the presence of.
     * @return true if the observer is registered with this dispatcher, false
     * otherwise.
     */
    public boolean contains(netty.smpp.api.event.ConnectionObserver ob) {
        return indexOf(ob) > -1;
    }
    ...
    public void notifyObservers(Connection conn, SMPPEvent event) {
        for (int i = 0; i < observers.length; i++) {
            netty.smpp.api.event.ConnectionObserver obs = observers[i];
            if (obs != null) {
                try {
    ...
    void growArray(netty.smpp.api.event.ConnectionObserver ob) {
        assert Thread.holdsLock(observers);
        netty.smpp.api.event.ConnectionObserver[] newArray =
                new netty.smpp.api.event.ConnectionObserver[observers.length + INCREMENT];
